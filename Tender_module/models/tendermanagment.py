...
class NewTender(models.Model):
	...
	name = fields.Char(string="Name of Company", required = True, search = "_search_name", index = True,
		track_visibility = True, states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	tend_street = fields.Char(string = "Street", required = True, 
		states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	tend_street2 = fields.Char(string = "Street2",index = True,
		states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	tend_city = fields.Char(string = "City", required = True, 
		states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	category = fields.Many2one('catelog.category', string = "Bid Title", required = True, 
		states={'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	publ_date = fields.Datetime(string = "Tender Published Date", required = True,
		states={'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	pre_bid = fields.Datetime(string = "Pre-Bid Meeting", 
		states={'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	pre_bid_location = fields.Char(string= "Location", 
		states={'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	# selection between days and date in tender dates
	field_type = fields.Selection([('days','Days'), ('date','Date')], 'Tender Submisson', default = "date",
		states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	subm_days = fields.Integer(string = "Expiry Days", states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	subm_date = fields.Datetime(string = "Tender Submisson Date", required = True, index = True, 
		states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	days_left = fields.Char(string = "Remaining Days", compute = '_check_days', 
		states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	tender_expire = fields.Integer(default = 0)
	bid_open_tech = fields.Datetime('Technical', states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	bid_open_fina = fields.Datetime('Financial', states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	# condition for percent or amount
	bond_amount_type = fields.Selection([('percentage','Percentage'), ('amount','Amount')], string = "Bid Bond Type", 
		default = "amount", store = True, states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	percentage = fields.Float(digits = 'Percentage', default = 0.0, required = True, 
		states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]},
		help = 'The percentage value should be in Decimal')
	amount = fields.Float(string = "Bid Security", required = True, 
		states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	bid_amount = fields.Float(string = "Bond Amount/Percentage", compute = '_check_value')
	bid_validity = fields.Date(string = "Bid Security Validity", 
		states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	bid_validity_days = fields.Char(compute = "_check_validity")
	# condition for percent or amount
	perform_bond_type = fields.Selection([('perform_bond_percent','Percentage'), ('perform_bond_amount','Amount')], string = "Performance Bond Type",
		default = "perform_bond_percent", store = True, states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	perform_bond_amount = fields.Float(string = "Performance Security Amount",
		states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	perform_bond_percent = fields.Float(digits = 'Percentage', default = 0.0,
		states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	...
	myprod_id = fields.One2many('tender.product', 'product_parent', string = "Order Products", 
	...
	bank_id = fields.Boolean(states={'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	contract_id = fields.Char(string = "IFB/Contract No. and Date", 
		states={'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	bid_doc_type = fields.Selection([('online','e-Bidding'), ('paperbased','HardCopy')], 'Bidding Option', default= 'paperbased',
		states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	bid_doc_price = fields.Float(string = "Amount of Bidding Document",
		states = {'cancel': [('readonly', True)], 'done': [('readonly', True)]})
	...
	def _check_perform_value(self):
		for perform_each in self:
			perform_each.perform_bond_show = perform_each.perform_bond_percent or perform_each.perform_bond_amount
	...
	def send_mail_template(self):
		...
		try:
		    template_id = ir_model_data.get_object_reference('Tender_module', 'tenderemail_templet')[1]
		except ValueError:
			template_id = False
		try:
...
class myproduct(models.Model):
	...
	specification = fields.Text(string = "Description", store = True, related = "prod_id.specification")
