...
class BankGuarantee(models.Model):
	...
	Performance_selection = fields.Selection(related = "tender_id.perform_bond_type", store = True)
	perform_bond_amount = fields.Float(related = "tender_id.perform_bond_amount", store = True)
	perform_bond_percent = fields.Float(related = "tender_id.perform_bond_percent", store = True)
	...
	purpose = fields.Many2one(related = "tender_id.category", store = True, string ="Contract(Purpose)")
	...
	def set_amt_in_words(self):
		self.amt_inwords = self.env.ref('base.NPR').with_context({'lang': 'en_US'}).amount_to_text(self.amount)
		self.amt_inwords += '\tOnly.'


	@api.depends('perform_bond_amount')
	def amount_into_text(self):
		self.amount_inwords = self.env.ref('base.NPR').with_context({'lang': 'en_US'}).amount_to_text(self.perform_bond_amount)
	...
	def reqst_bank_guarantee(self):
		...
		try:
		    template_bank_id = ir_model_data.get_object_reference('Tender_module', 'bank_gearantee_request_email')[1]
		except ValueError:
